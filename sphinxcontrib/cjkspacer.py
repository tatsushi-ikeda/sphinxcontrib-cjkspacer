...
class CJKBoundary(object):
    ...
    # The above expressions are generated by the following emacs-lisp code
    # (See also README.md)
    # (setf cjk-list
    ...
    def __init__(self, cjk_pattern, b_except_pattern, a_except_pattern):
        self._border_pattern_pairs = (
            (
                re.compile(
                    "(?:[^{b_except}])$".format(b_except=b_except_pattern + cjk_pattern)
                ),
            ...
            (
                re.compile("(?:[{cjk}])$".format(cjk=cjk_pattern)),
                re.compile(
                    "^(?:[^{a_except}])".format(a_except=a_except_pattern + cjk_pattern)
                ),
            ),
        )
        self._pattern = re.compile(
            "(?<![{b_except}])(?=[{cjk}])|(?<=[{cjk}])(?![{a_except}])".format(
                cjk=cjk_pattern,
    ...
    def check_boundary(self, leading_txt, following_txt):
        return any(
            (pat[0].search(leading_txt) and pat[1].match(following_txt))
            ...
            new_child = list(
                chain.from_iterable(
                    zip_longest(
                        (
                            nodes.Text(sep)
                            for sep in self._cjk_boundary.split(new_txt)
                            if sep
                        ),
                        [],
                        fillvalue=self._spacer,
                    )
                )
            )[:-1]
...
def insert_cjkspacer(app, doctree, _docname):
    ...
    visitor = CJKSpacerVisitor(
        doctree,
        cjkspacer(spacer_str),
        CJKBoundary(
            app.config.cjkspacer_cjk_characters,
...
def setup(app):
    app.add_config_value(
        "cjkspacer_spacer", {"html": '<span class="cjkspacer"></span>'}, "env", dict
    )
    app.add_config_value(
        "cjkspacer_cjk_characters", CJKBoundary.CJK_CHARACTERS, "env", str
    )
    app.add_config_value(
        "cjkspacer_before_exceptions", CJKBoundary.BEFORE_EXCEPTIONS, "env", str
    )
    app.add_config_value(
        "cjkspacer_after_exceptions", CJKBoundary.AFTER_EXCEPTIONS, "env", str
    )
    # formats, dict
